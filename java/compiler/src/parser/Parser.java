...
*/
    public AST rStatement() throws SyntaxError {
        ...
        //repeat identifier with a new rule is added
        //for the project. repeat has a kid of block
        //within the while expression.
        if(isNextTok(Tokens.Repeat))
        {
            scan();
            t = new RepeatTree();//creates the appropriate tree
            t.addKid(rBlock());//Block
            expect(Tokens.While);//While Token
            t.addKid(rExpr());//Expression
            return t;
        }    
...
*/
    public AST rFactor() throws SyntaxError {
        ...
        //****************************************
        //****************************************
        else if (isNextTok(Tokens.Float)) {  //  -> <float>
