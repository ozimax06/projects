...
public class Lexer {
    ...
    char floatCh = '.';//indictaes that charcter is a float
    ...
    {
      Token tok;
      String argument = args[0];//Command line argument
      //The test expression is in simple.x file
      try 
      {
        Lexer lex = new Lexer(argument);
         while (true) 
            {
                tok = lex.nextToken();
                //Token line number will return the line numbers
                String p = ""+ Token.lineNumber+" L: " + tok.getLeftPosition() +
...
*/
    public Token nextToken() { // ch is always the next char to process
        ...
        if (Character.isJavaIdentifierStart(ch)) {
         ...
         //Recognizes the next Token and if it satisfies the conditions
         //of a float token, then the program recognizes it, calls the
         //newIdToken.(Since Float is an Identifier)
         if (Character.isDigit(ch)||ch==floatCh) //scans the first charcter
         {
            String floatNumber = "";
            boolean isFloat = false;
            int numberOfDots = 0;//program makes sure that token has only one
            //float dot. Otherwise, separates the token.
            
            try 
            {
                do {
                    if((ch==floatCh)&&(numberOfDots==1))
                    {
                        //Returns the float and separates the token
                        return newIdToken(floatNumber,startPosition,endPosition);
                    }    
                    else if(ch==floatCh) 
                    {    
                       isFloat = true; 
                       numberOfDots++;//Program keeps track on the dots
                    }
                    
                    endPosition++;
                    floatNumber += ch;
                    ch = source.read();
                } while ((Character.isDigit(ch))||(ch==floatCh));
            }//end of try
            catch (Exception e) {
                atEOF = true;
            }
            if(isFloat)//If satisfies the condition, nextToken is returned
                return newIdToken(floatNumber,startPosition,endPosition);
        }
